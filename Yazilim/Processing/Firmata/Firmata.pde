...
void setup() {
  ...
  //serial = new Serial(this, "/dev/tty.usbserial-A603JL3X", 115200);
  ...
  arduino.pinMode(d_motor, Arduino.OUTPUT);
  arduino.pinMode(d_motor2, Arduino.OUTPUT);
  arduino.pinMode(d_motor3, Arduino.OUTPUT);
  arduino.pinMode(d_motor4, Arduino.OUTPUT);
    
  arduino.pinMode(led_k_1, Arduino.OUTPUT);
  arduino.pinMode(led_k_2, Arduino.OUTPUT);
  arduino.pinMode(led_y_1, Arduino.OUTPUT);
  arduino.pinMode(led_y_2, Arduino.OUTPUT);
...
void draw() {
  ...
  //serial.write(int(buzzer));
  
  if(motor_ters == 1){
    arduino.digitalWrite(d_motor, Arduino.HIGH);
    arduino.digitalWrite(d_motor2, Arduino.HIGH);
    arduino.digitalWrite(d_motor3, Arduino.LOW);
    arduino.digitalWrite(d_motor4, Arduino.LOW);
  }else{
    arduino.digitalWrite(d_motor, Arduino.LOW);
    arduino.digitalWrite(d_motor2, Arduino.LOW);
    arduino.digitalWrite(d_motor3, Arduino.HIGH);
    arduino.digitalWrite(d_motor4, Arduino.HIGH);
  }
  
  arduino.analogWrite(motor, int(motor_sag));
  arduino.analogWrite(motor2, int(motor_sag));
  arduino.analogWrite(motor3, int(motor_sol));
  arduino.analogWrite(motor4, int(motor_sol));
  
  if(int(motor_sag) == 0 && int(motor_sol) == 0){
    arduino.digitalWrite(led_k_1, Arduino.HIGH);
    arduino.digitalWrite(led_k_2, Arduino.HIGH);
    arduino.digitalWrite(led_y_1, Arduino.LOW);
    arduino.digitalWrite(led_y_2, Arduino.LOW);
  }else{
    arduino.digitalWrite(led_k_1, Arduino.LOW);
    arduino.digitalWrite(led_k_2, Arduino.LOW);
    arduino.digitalWrite(led_y_1, Arduino.HIGH);
    arduino.digitalWrite(led_y_2, Arduino.HIGH);
  }
